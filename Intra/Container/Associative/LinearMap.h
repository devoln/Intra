...
public:
	...
	typedef Intra::Range::RZipKV<CSpan<K>, Span<V>> ElementRange;
	typedef Intra::Range::RZipKV<CSpan<K>, CSpan<V>> ElementConstRange;
	...
	void Rename(const K& key, const K& newKey)
	{
		...
		INTRA_DEBUG_ASSERT(!Range::Contains(mKeys, newKey));
		auto i = FindIndex(key);
		if(i==Count()) return;
		mKeys[i] = newKey;
	}
	...
	forceinline bool KeyExists(const K& key) const {return Range::Contains(mKeys, key);}
