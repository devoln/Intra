...
/*#else
	...
	if(lError==ERROR_SUCCESS)
	{
		...
		if(lError==ERROR_SUCCESS) result.BrandString = Range::TrimRight(StringView(processorName, size), '\0');
	}
...
ProcessorInfo ProcessorInfo::Get()
{
	...
	StringView modelNameLine = Range::Find(allCpuInfo, "\nmodel name");
	result.BrandString = Range::TakeUntil(Range::Find(modelNameLine, ':').Drop(2), '\n');

	StringView cpuCoresLine = Range::Find(allCpuInfo, "\ncpu cores");
	result.CoreNumber = Range::Parse<ushort>(Range::TakeUntil(Range::Find(cpuCoresLine, ':').Drop(2), '\n'));
	...
	StringView cpuMHzLine = Range::Find(allCpuInfo, "\ncpu MHz");
	StringView cpuMHzStr = Range::TakeUntil(Range::Find(cpuMHzLine, ':').Drop(2), '\n');
