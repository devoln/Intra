...
template<typename R, typename C = TLess> INTRA_CONSTEXPR2 Requires<
	...
	CAssignableRange<R>
> InsertionSort(const R& range, C comparer = Less)
{
...
INTRA_CONSTEXPR2 Requires<
	CRandomAccessRangeWithLength<R> &&
	CAssignableRange<R>
> ShellSort(const R& range, C comparer = Less)
{
	const size_t count = Count(range);
...
> INTRA_CONSTEXPR2 forceinline Requires<
	...
	CRandomAccessRangeWithLength<AsR> &&
	CAssignableRange<AsR>
> ShellSort(R&& range, C comparer = Less)
