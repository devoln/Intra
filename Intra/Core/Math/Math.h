...
constexpr inline int FixedPointLog(uint x)
{
	int y = 0xa65af;
...
template<typename T> INTRA_NODISCARD constexpr T PowInt(T x, int y)
{
	uint n = uint(Abs(y));
	for(T z = T(1); ; x *= x)
	{
...
template<typename T> INTRA_MATH_CONSTEXPR T Erf1(T x)
{
...
template<typename T> INTRA_NODISCARD INTRA_MATH_CONSTEXPR T Erfc2(T x)
{
...
template<typename T> INTRA_NODISCARD INTRA_MATH_CONSTEXPR T Erfc3(T x)
{
...
INTRA_NODISCARD constexpr inline uint GreatestCommonDivisor(uint a, uint b)
{
	while(a != b)
	{
		if(a > b) a -= b;
...
INTRA_NODISCARD constexpr inline uint CeilToNextPow2(uint v)
{
	v--;
	v |= v >> 1;
	v |= v >> 2;
...
INTRA_NODISCARD constexpr inline byte Log2i(uint x)
{
