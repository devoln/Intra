...
INTRA_CONSTEXPR2 DecimalFloat<uint32> f2d(uint32 ieeeMantissa, uint32 ieeeExponent)
{
	...
	if(e2 >= 0)
	{
		...
		e10 = int32(q);
		const int32 k = D::FLOAT_POW5_INV_BITCOUNT + Pow5bits(int32(q)) - 1;
		const int32 i = -e2 + int32(q) + k;
		vr = D::MulPow5InvDivPow2(mv, q, i);
		vp = D::MulPow5InvDivPow2(mp, q, i);
		vm = D::MulPow5InvDivPow2(mm, q, i);
		if(q != 0 && (vp - 1) / 10 <= vm / 10) {
			// We need to know one removed digit even if we are not going to loop below. We could use
			// q = X - 1 above, except that would require 33 bits for the result, and we've found that
			// 32-bit arithmetic is faster even on 64-bit machines.
			const int32 l = D::FLOAT_POW5_INV_BITCOUNT + Pow5bits(int32(q - 1)) - 1;
			lastRemovedDigit = byte(D::MulPow5InvDivPow2(mv, q - 1, -e2 + int32(q) - 1 + l) % 10);
		}
		if(q <= 9) {
	...
	else
	{
		...
		const int32 k = Pow5bits(i) - D::FLOAT_POW5_BITCOUNT;
		int32 j = int32(q) - k;
		vr = D::MulPow5divPow2(mv, uint32(i), j);
		vp = D::MulPow5divPow2(mp, uint32(i), j);
		vm = D::MulPow5divPow2(mm, uint32(i), j);
		if(q != 0 && (vp - 1) / 10 <= vm / 10)
		{
			j = int32(q) - 1 - Pow5bits(i + 1) - D::FLOAT_POW5_BITCOUNT;
			lastRemovedDigit = byte(D::MulPow5divPow2(mv, uint32(i + 1), j) % 10);
		}
		if(q <= 1)
		{
