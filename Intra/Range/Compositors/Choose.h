...
{
private:
	typedef Meta::CommonTypeRef<Concepts::ReturnValueTypeOf<R1>, Concepts::ReturnValueTypeOf<R2>> ReturnValueType;
public:
	enum: bool {RangeIsFinite = Concepts::CommonRangeCategoryAllFinite<R1, R2>::Finite};

	RChoose(null_t=null): mRange1(null), mRange2(null) {}

	RChoose(R1&& ifFalseRange, R2&& ifTrueRange, bool condition):
		mRange1(condition? null: Cpp::Move(ifFalseRange)),
		mRange2(condition? Cpp::Move(ifTrueRange): null) {}
...
template<typename R1, typename R2> forceinline Meta::EnableIf<
	!Meta::TypeEqualsIgnoreCVRef<Concepts::RangeOfType<R1>, Concepts::RangeOfType<R2>>::_,
RChoose<Concepts::RangeOfTypeNoCRef<R1>, Concepts::RangeOfTypeNoCRef<R2>>>
...
template<typename R1, typename R2> forceinline Meta::EnableIf<
	Meta::TypeEqualsIgnoreCVRef<Concepts::RangeOfType<R1>, Concepts::RangeOfType<R2>>::_,
Concepts::RangeOfType<R1&&>> Choose(R1&& ifFalseRange, R2&& ifTrueRange, bool condition)
