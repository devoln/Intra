...
{
private:
	typedef Meta::Tuple<Concepts::ReturnValueTypeOf<RANGES>...> ReturnValueType;
...
public:
	...
	template<typename R0, typename... RANGES1> forceinline RZip(R0&& r0, RANGES1&&... ranges):
		OriginalRanges(Cpp::Forward<R0>(r0), Cpp::Forward<RANGES1>(ranges)...) {}
...
template<typename R0, typename... RANGES, typename=Meta::EnableIf<
	Concepts::IsAsAccessibleRange<R0>::_>> forceinline
RZip<Concepts::RangeOfTypeNoCRef<R0>, Concepts::RangeOfTypeNoCRef<RANGES>...> Zip(R0&& range0, RANGES&&... ranges)
