...
template<typename T> forceinline T Set(TScalarOf<T> v) noexcept
{
	T result;
...
forceinline int4 Load4(const int* src) {return *reinterpret_cast<const int4*>(src);}
forceinline int4 Load4Aligned(const int* src) {return *reinterpret_cast<const int4*>(INTRA_ASSUME_ALIGNED(src, sizeof(int4));}
forceinline float4 Load4(const float* src) {return *reinterpret_cast<const float4*>(src);}
forceinline float4 Load4Aligned(const float* src) {return *reinterpret_cast<const float4*>(INTRA_ASSUME_ALIGNED(src, sizeof(float4));}
forceinline int8 Load8(const int* src) {return *reinterpret_cast<const int8*>(src);}
forceinline int8 Load8Aligned(const int* src) {return *reinterpret_cast<const int8*>(INTRA_ASSUME_ALIGNED(src, sizeof(int8));}
forceinline float8 Load8(const float* src) {return *reinterpret_cast<const float8*>(src));}
forceinline float8 Load8Aligned(const float* src) {return *reinterpret_cast<const float8*>(INTRA_ASSUME_ALIGNED(src, sizeof(float8));}
template<typename T> forceinline void Store(T v, TScalarOf<T>* dst) {*reinterpret_cast<T*>(dst) = v;}
template<typename T> forceinline void StoreAligned(T v, TScalarOf<T>* dst) {*reinterpret_cast<T*>(INTRA_ASSUME_ALIGNED(dst, sizeof(T))) = v;}
...
template<int n, typename T> forceinline Requires<sizeof(T)/sizeof(T{}[0]) == 4, T> RotateLeft(const T& v) noexcept
{
	return Shuffle<n & 3, (n + 1) & 3, (n + 2) & 3, (n + 3) & 3>(v);
}

template<int n, typename T> forceinline Requires<sizeof(T)/sizeof(T{}[0]) == 8, T> RotateLeft(const T& v) noexcept
{
...
forceinline float4 INTRA_VECTORCALL InterleaveLow(float4 a, float4 b) noexcept {return float4{a[0], b[0], a[1], b[1]};}
forceinline int4 INTRA_VECTORCALL InterleaveLow(int4 a, int4 b) noexcept {return int4{a[0], b[0], a[1], b[1]};}
forceinline float4 INTRA_VECTORCALL InterleaveHigh(float4 a, float4 b) noexcept {return float4{a[2], b[2], a[3], b[3]};}
forceinline int4 INTRA_VECTORCALL InterleaveHigh(int4 a, int4 b) noexcept {return int4{a[2], b[2], a[3], b[3]};}

forceinline float8 INTRA_VECTORCALL InterleaveLow(float8 a, float8 b) noexcept {return float8{a[0], b[0], a[1], b[1], a[2], b[2], a[3], b[3]};}
forceinline int8 INTRA_VECTORCALL InterleaveLow(int8 a, int8 b) noexcept {return int8{a[0], b[0], a[1], b[1], a[2], b[2], a[3], b[3]};}
forceinline float8 INTRA_VECTORCALL InterleaveHigh(float8 a, float8 b) noexcept {return float8{a[4], b[4], a[5], b[5], a[6], b[6], a[7], b[7]};}
forceinline int8 INTRA_VECTORCALL InterleaveHigh(int8 a, int8 b) noexcept {return int8{a[4], b[4], a[5], b[5], a[6], b[6], a[7], b[7]};}

forceinline void End() noexcept {}

forceinline int4 INTRA_VECTORCALL UnsignedRightBitShift(int4 x, int bits) noexcept
{
	return int4{int(unsigned(x[0]) >> bits), int(unsigned(x[1]) >> bits), int(unsigned(x[2]) >> bits), int(unsigned(x[3]) >> bits)};
}

forceinline int8 INTRA_VECTORCALL UnsignedRightBitShift(int8 x, int bits) noexcept
{
	return int8{
