...
static constexpr struct {uint fourcc; ImageFormat::I format;} FourCC_ImageFormat[]={
...
static bool has_dx10_header(const DDS_PIXELFORMAT& ddspf)
{
	return (ddspf.flags & DDPF_FOURCC) &&
		Misc::BitsEqual(ddspf.fourCC, "DX10", LengthOf(ddspf.fourCC)) == 0;
}

bool LoaderDDS::IsValidHeader(const void* header, size_t headerSize) const
{
	if(headerSize < 4) return false;
	return Misc::BitsEqual(reinterpret_cast<const char*>(header), "DDS ", 4);
}
