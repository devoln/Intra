...
ServerSocket::ServerSocket(SocketType type, ushort port, size_t maxConnections, ErrorReporter err)
{
	initContext();
...
StreamSocket::StreamSocket(SocketType type, StringView host, ushort port, ErrorReporter err):
	BasicSocket(type, err)
{
	...
	if(gaiErr != 0)
	{
		err.Error({uint32(gaiErr)}, gai_strerror(gaiErr), INTRA_SOURCE_INFO);
		Close();
		return;
	}
