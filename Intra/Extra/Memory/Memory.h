...
> Initialize(Span<T> dst)
{
	Misc::C::memset(dst.Begin, 0, dst.Length()*sizeof(T));
}
...
#ifdef INTRA_DEBUG
	if(dst.Empty()) return;
	Misc::C::memset(dst.Data(), 0xDE, dst.Length()*sizeof(dst.First()));
#endif
	(void)dst;
}
...
{
#ifdef INTRA_DEBUG
	Misc::C::memset(&dst, 0xDE, sizeof(T));
#endif
	(void)dst;
}
...
{
	INTRA_PRECONDITION(Count(dst) >= Count(src));
	auto dstCopy = dst;
	auto srcCopy = src;
	while(!srcCopy.Empty())
	{
...
template<typename OR, typename R> constexpr forceinline Requires<
	CArrayClass<OR> &&
	CArrayClass<R> &&
...
> CopyRecreate(Span<T> dst, CSpan<T> src)
{
	...
	{
		dst.First().~T();
		new(Construct, &dst.First()) T(src.First());
		src.PopFirst();
	}
}
...
{
	INTRA_PRECONDITION(dst.Length() == src.Length());
	Misc::CopyBits(dst.Data(), src.Data(), dst.Length());
}
...
> MoveInit(Span<T> dst, Span<T> src)
{
	...
	while(!dst.Empty())
	{
		new(Construct, &dst.First()) T(Move(src.First()));
		dst.PopFirst();
		src.PopFirst();
	}
}
