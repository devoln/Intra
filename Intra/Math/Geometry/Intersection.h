...
template<typename T> INTRA_EXTENDED_CONSTEXPR short ClassifySphere(const Plane<T>& plane, const Sphere<T>& sphere) const noexcept
{
	const T dist = Dot(sphere.Center, plane.Normal) + plane.D;
