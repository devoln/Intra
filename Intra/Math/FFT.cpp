...
#include "Core/Range/Mutation/Transform.h" //TODO: use SIMD?
...
{
	INTRA_DEBUG_ASSERT(IsPow2(data.Length()));
	rearrangeData(data);
...
void InplaceFFT(Span<float> real, Span<float> imag)
{
	...
	INTRA_DEBUG_ASSERT(IsPow2(real.Length()));
...
void InplaceInverseFFT(Span<float> real, Span<float> imag)
{
	...
	// TODO: use Simd
	Transform(imag, normalize);
	Transform(real, normalize);
}
