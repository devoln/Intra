...
#define INTRA_SOURCE_INFO ::Intra::Utils::SourceInfo{INTRA_CURRENT_FUNCTION, __FILE__, unsigned(__LINE__)}
...
//! В результате выполнения функции buf указывает на свободный остаток буфера.
StringView BuildDiagnosticMessage(Span<char>& buf, StringView type, StringView func, StringView file,
...
//! @return Ссылка на сообщение об ошибке, записанное в буфере buf, как StringView.
//! В результате выполнения функции buf указывает на свободный остаток буфера.
FixedArray<char> BuildAppendDiagnosticMessage(StringView initStr, StringView type, StringView func, StringView file,
	unsigned line, StringView msg, size_t stackFramesToSkip = 0);

//! Вывести сообщение о критической ошибке на экран и аварийно завершить программу.
void FatalErrorMessageAbort(const SourceInfo&, StringView msg, bool printStackTrace);
void FatalErrorMessageAbort(const SourceInfo&, StringView msg);

//! Функция, которая вызывается при FatalErrorMessageAbort, например когда срабатывает ассерт.
typedef void(*FatalErrorCallbackType)(const SourceInfo&, StringView msg);
...
void CallFatalErrorCallback(const SourceInfo&, StringView msg);
void CallFatalErrorCallback(const SourceInfo&, const char* msg);


#define INTRA_FATAL_ERROR(msg) (INTRA_DEBUGGER_BREAKPOINT, \
	::Intra::Utils::CallFatalErrorCallback(INTRA_SOURCE_INFO, msg))
...
#define INTRA_DEBUGGER_BREAKPOINT (::Intra::Utils::IsDebuggerAttached()? (INTRA_DEBUG_BREAK, true): true)
...
#define INTRA_ASSERT_WARNING(expression) ((expression) || (INTRA_DEBUGGER_BREAKPOINT && \
    (::Intra::Utils::PrintDebugMessage("Warning (" __FILE__ ":" + ::Intra::StringOf(__LINE__) + " in function " + \
        ::Intra::Utils::StringView(__FUNCTION__) + "): assertion (" # expression ") failed!\r\n" __FILE__ "\r\n"), false)))
...
#define INTRA_NAN_CHECK(val) {if(val == ::Intra::Cpp::NaN) {(val)=0;}}
