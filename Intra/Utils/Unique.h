...
template<typename T> struct Unique
{
	...
	*/
	...
	INTRA_CONSTEXPR2 forceinline Unique& operator=(Unique&& rhs) noexcept
	{
		Swap(mPtr, rhs.mPtr);
		return *this;
	}
	...
	INTRA_NODISCARD INTRA_CONSTEXPR2 forceinline T& operator*() const {INTRA_DEBUG_ASSERT(mPtr != null); return *mPtr;}
	INTRA_NODISCARD INTRA_CONSTEXPR2 forceinline T* operator->() const {INTRA_DEBUG_ASSERT(mPtr != null); return mPtr;}
...
template<typename T> constexpr bool IsTriviallyRelocatable<Utils::Unique<T>> = true;
}
INTRA_END
