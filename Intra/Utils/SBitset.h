...
private:
	...
	template<typename T, size_t BitsPerItem> INTRA_CONSTEXPR2 void copyFrom(Span<T> arr)
	{
...
public:
	...
	>> INTRA_CONSTEXPR2 forceinline SBitset(Span<T> arr): v{}
	{copyFrom<T, sizeof(T)*8>(arr);}

	INTRA_CONSTEXPR2 forceinline SBitset(CSpan<bool> arr): v{}
	...
	*/
	INTRA_CONSTEXPR2 forceinline SBitset(InitializerList<uint64> arr): v{}
	...
	*/
	...
	INTRA_CONSTEXPR2 forceinline void Set(size_t index)
	{
		INTRA_DEBUG_ASSERT(index < N);
		v[index >> indexShift] |= basicType(basicType(1) << (index & indexMask));
	}

	INTRA_CONSTEXPR2 forceinline void Reset(size_t index)
	{
		INTRA_DEBUG_ASSERT(index < N);
		v[index >> indexShift] &= basicType(~(basicType(1) << (index & indexMask)));
	}
	...
	INTRA_CONSTEXPR2 SBitset& operator=(null_t) noexcept
	{
	...
	INTRA_CONSTEXPR2 bool operator==(null_t) const noexcept
	{
		basicType res = 0;
		for(size_t i = 0; i < nWords; i++)
			res |= v[i];
	...
	INTRA_CONSTEXPR2 forceinline bool operator!=(null_t) const noexcept {return !operator==(null);}

	INTRA_CONSTEXPR2 bool operator==(const SBitset& rhs) const noexcept
	{
		basicType res = 0;
		for(size_t i = 0; i < nWords; i++)
			res |= v[i] ^ rhs.v[i];
	...
	INTRA_CONSTEXPR2 forceinline bool operator!=(const SBitset& rhs) const noexcept {return !operator==(rhs);}

	//! Set union
	INTRA_CONSTEXPR2 SBitset operator|=(const SBitset& rhs) noexcept
	{
		for(size_t i = 0; i < nWords; i++)
			v[i] |= rhs.v[i];
		return *this;
	...
	//! Set union
	INTRA_CONSTEXPR2 SBitset operator|(SBitset rhs) const noexcept
	{return rhs |= *this;}
...
static_assert(~SBitset<3>(SpanOf({false, false, true})) == SBitset<3>(SpanOf({true, true, false})), "TEST FAILED!");
#endif
