...
public:
	Range::FlatConstStringRange FieldNames(const byte* blockEnd) const
	{
		...
		const size_t sizeInBytes = size_t(Algo::ParseVarUInt(s));
	...
	forceinline size_t FieldCount(const byte* blockEnd) const {return Range::Count(FieldNames(blockEnd));}
...
const RawType& ParseType(CSpan<byte>& range)
{
	...
	{
		const ulong64 divisor = Algo::ParseVarUInt(range);
		return;
	}

	if(signature == RawType::FixedArray)
	{
		const ulong64 sizeInBytes = Algo::ParseVarUInt(range);
		ParseType(range);
		return;
	}
	...
	{
		const ulong64 elementCount = Algo::ParseVarUInt(range);
		return;
	}
