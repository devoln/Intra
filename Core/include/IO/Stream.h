...
public:
	...
	Array<T>> ReadArray(size_t count)
	{
		Array<T> result;
...
public:
	...
	template<typename T> Meta::EnableIf<
		Meta::IsAlmostPod<T>::_
	> Write(const T& value) {WriteData(&value, sizeof(T));}
	...
	template<typename Arg0> void Print(Arg0&& t)
	{
		Print(StringView(ToString(Meta::Forward<Arg0>(t))));
	}
	...
	{
		Print(Meta::Forward<Args>(args)..., endl);
	}
