...
public:
	...
	void WriteRaw(const void* src, size_t bytes)
	{
		...
		C::memmove(Rest.Data(), src, bytes);
		Rest.PopFirstExactly(bytes);
	}
	...
	forceinline void WriteShortRaw(StringView src)
	{
		...
		while(srcPtr<srcEnd) Rest.Put(*srcPtr++);
	}
	...
	{
		INTRA_ASSERT(Begin <= Rest.Data());
		return StringView(Begin, Rest.Data());
	}
	...
	{
		INTRA_ASSERT(Begin <= Rest.Data());
		return ArrayRange<const char>(Begin, Rest.Data());
	}
...
public:
	...
	template<typename T> forceinline Meta::EnableIf<
		Meta::IsAlmostPod<T>::_
	> WriteRaw(const T& src) {src; counter.Counter += sizeof(T);}
...
public:
	...
	template<typename T> forceinline Meta::EnableIf<
		Meta::IsAlmostPod<T>::_
	> ReadRaw(T& dst) {ReadRaw(&dst, sizeof(T));}
	...
	{
		return Algo::TakeRecursiveBlockAdvance(Rest, counter, null,
