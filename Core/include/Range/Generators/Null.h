...
template<typename T> struct NullRange
{
	...
	T First() const {INTRA_DEBUG_ASSERT(false); return T();}
	T Last() const {INTRA_DEBUG_ASSERT(false); return T();}
	void PopFirst() {INTRA_DEBUG_ASSERT(false);}
	void PopLast() {INTRA_DEBUG_ASSERT(false);}
	T operator[](size_t) {INTRA_DEBUG_ASSERT(false); return T();}
