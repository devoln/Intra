...
template<typename R0, typename R1, typename... Rs> struct AllHasLength<R0, R1, Rs...>: Meta::TypeFromValue<bool,
	HasLength<R0>::_ && AllHasLength<R1, Rs...>::_
> {};
...
struct Fronter
{
	template<typename T> forceinline ReturnValueTypeOf<T> operator()(const T& range) const
	{return range.First();}
};
...
{
	size_t start, end;
	template<typename T> forceinline SliceTypeOf<T> operator()(const T& range) const
