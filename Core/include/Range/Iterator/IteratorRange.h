...
template<typename I1, typename I2> struct IteratorRange
{
	...
	decltype(*Meta::Val<I1>()) First() const {INTRA_ASSERT(!Empty()); return *Begin;}

	template<typename U=I2> forceinline Meta::EnableIf<
		IteratorConcepts::IsMinimalBidirectionalIterator<U>::_
	> PopLast() {INTRA_ASSERT(!Empty()); --End;}

	template<typename U=I2> forceinline Meta::EnableIf<
		IteratorConcepts::IsMinimalBidirectionalIterator<U>::_,
	decltype(*Meta::Val<I2>())> Last() const {INTRA_ASSERT(!Empty()); return *--I2(End);}

	template<typename U=I2> forceinline Meta::EnableIf<
		IteratorConcepts::HasDifference<U, I1>::_,
	size_t> Length() const {INTRA_ASSERT(!Empty()); return size_t(End-Begin);}
