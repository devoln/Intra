...
/*#else
	...
	if(lError==ERROR_SUCCESS)
	{
		...
		if(lError==ERROR_SUCCESS) result.BrandString = Algo::TrimRight(StringView(processorName, size), '\0');
	}
...
ProcessorInfo ProcessorInfo::Get()
{
	...
	StringView cpuCoresLine = Algo::Find(allCpuInfo, "\ncpu cores");
	result.CoreNumber = Algo::Parse<ushort>(Range::TakeUntil(Algo::Find(cpuCoresLine, ':').Drop(2), '\n'));

	result.LogicalProcessorNumber = ushort(Algo::Count(allCpuInfo, "\nprocessor"));
	if(Algo::StartsWith(allCpuInfo, "processor")) result.LogicalProcessorNumber++;

	if(result.CoreNumber==0) result.CoreNumber = result.LogicalProcessorNumber;

	StringView cpuMHzLine = Algo::Find(allCpuInfo, "\ncpu MHz");
	StringView cpuMHzStr = Range::TakeUntil(Algo::Find(cpuMHzLine, ':').Drop(2), '\n');
	result.Frequency = ulong64(1000000*Algo::Parse<double>(cpuMHzStr));
