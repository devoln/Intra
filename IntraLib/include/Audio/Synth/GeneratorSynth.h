...
template<typename T> struct SamplerPassParams
{
	...
	SamplerPassParams(T&& generator, ushort harmonics, float scale, float freqMultiplyer):
		Generator(Meta::Move(generator)), Harmonics(harmonics), Scale(scale), FreqMultiplyer(freqMultiplyer) {}
	...
	float freq, float volume, ArrayRange<float> outSamples, uint sampleRate, bool add)
{
	...
	//—читаем громкость основной гармоники так, чтобы суммарна€ громкость по всем гармоникам не превышала scale
