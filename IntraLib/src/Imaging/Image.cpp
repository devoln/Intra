...
Image Image::FromStream(IO::IInputStream& stream, size_t bytes)
{
...
FileFormat Image::DetectFileFormatByHeader(byte header[12])
{
	for(auto& loader: AImageLoader::GetRegisteredLoaders())
...
ImageInfo Image::GetImageInfo(IInputStream& stream, FileFormat* oFormat)
{
	INTRA_ASSERT(stream.IsSeekable());
	auto startPos = stream.GetPos();
...
ImageInfo Image::GetImageInfo(StringView filename, FileFormat* oFormat)
{
