...
## Эксперимент 13: fast string onset вместо старого bloom — ПРИНЯТО

Диагностический SF2 был сдвинут по каждой зоне ровно к
`PianoRegionData::DecayOnset`, то есть к той временной точке, относительно
которой joint temporal fitter определяет табличные `Amp + Decay1..4`.
Несмотря на удалённую начальную часть семпла, такой SF2 стартовал заметно
чётче текущего synth.

Причина оказалась в runtime: табличный Amp уже описывает сформированное
состояние струны в `DecayOnset`, но старый путь снова выращивал его из нуля
через contact/body/push/bloom и ручной `keyScale` (местами 0.06..0.35).
Это делало фронт ноты мягким и «ватным».

Принятый вариант:

- все измеренные partial'ы стартуют из нулевой выходной амплитуды, сохраняя
  табличные Phase/Freq;
- полный harmonic stack достигает ~99.5% за ~1 ms;
- сразу после этого включается Decay1;
- Decay2/3/4 сохраняют свои исходные длительности относительно измеренного
  `DecayOnset`;
- legacy contact/body/push/bloom overlays отключены;
- отдельная hammer/contact модель будет строиться позже поверх уже чёткой
  струны, а не использоваться для компенсации мягкого sustain-onset.

Первый экспериментальный fast-onset был ошибочным: `mDecayOnsetSamples=0`
обнулял `mAtk` до первого сэмпла, поэтому оставался почти один overlay.
Исправленный вариант с 1 ms ramp пользователь сравнил с cropped SF2 и оценил
атаку как практически идентичную по чёткости: это «совсем другое дело» по
сравнению со старой ватной атакой.
