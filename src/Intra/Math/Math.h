...
template<typename T> constexpr T Sqr(T n) noexcept {return n*n;}
template<typename T> constexpr T Abs(T v) noexcept {return v >= 0? v: -v;}

template<typename T> constexpr T Min(const T& t1, const T& t2) noexcept {return t1 < t2? t1: t2;}
template<typename T> constexpr T Max(const T& t1, const T& t2) noexcept {return t1 < t2? t2: t1;}
...
constexpr auto Clamp(T v, N minv, X maxv) noexcept {return Max(minv, Min(maxv, v));}

//! Find the nearest integer less than or equal to x.
//! x must be in range [-2^31; 2^31), otherwise return value is undefined.
constexpr int IFloor(float x) noexcept {return int(x) - int(x < 0 && x != float(int(x)));}
...
constexpr int64 IFloor(double x) noexcept {return int64(x) - int64(x < 0 && x != double(int64(x)));}
#if INTRA_CONSTEXPR_TEST
...
constexpr inline int FixedPointLog(unsigned x)
{
	int y = 0xa65af;
...
static_assert(FixedPointLog(unsigned(Constants.E*65536)) == 65537, "ERROR"); // must be 65536, rounding error?
#endif
...
[[nodiscard]] INTRA_MATH_CONSTEXPR INTRA_FORCEINLINE double Fract(double x) {return x - Floor(x);}
[[nodiscard]] INTRA_MATH_CONSTEXPR INTRA_FORCEINLINE float Fract(float x) {return x - Floor(x);}

template<typename T> [[nodiscard]] constexpr T PowInt(T x, int y)
{
...
template<typename T> [[nodiscard]] INTRA_MATH_CONSTEXPR INTRA_FORCEINLINE T Pow2(T x) {return Exp(x*T(0.6931472));}
...
template<typename T> T Erf(T x)
{
	...
	if(xAbs <= T(0.46875)) return Erf1(x);
	if(xAbs <= 4) return Sign(x)*(1 - Erfc2(xAbs));
	return Sign(x)*(1 - Erfc3(xAbs));
}

template<typename T> INTRA_FORCEINLINE T Erfc(T x) {return 1 - Erf(x);}
...
[[nodiscard]] inline float FastByteToNormalizedFloat(byte x)
{
...
[[nodiscard]] constexpr inline uint32 GreatestCommonDivisor(uint32 a, uint32 b)
{
	while(a != b)
	{
		if(a > b) a -= b;
...
template<typename T, typename U> [[nodiscard]] constexpr T LinearMix(T x, T y, U factor) {return T(x*(U(1) - factor) + y*factor);}

template<typename T> [[nodiscard]] constexpr T Step(T edge, T value) {return T(value >= edge);}
...
[[nodiscard]] constexpr inline uint32 CeilToNextPow2(uint32 v)
{
	v--;
	v |= v >> 1u;
	v |= v >> 2u;
...
[[nodiscard]] constexpr inline byte Log2i(uint32 x)
{
...
[[nodiscard]] constexpr bool IsPow2(size_t x) {return x != 0 && ((x & (x - 1)) == 0);}
