...
template<size_t N> class SBitset
{
	...
	using basicType = TIntMin<nBytesPerWord>;
	...
	static constexpr basicType remainderMask = basicType(~basicType()) >> basicType(remainderBits == 0? 0: (sizeof(basicType)*8 - remainderBits));
	basicType v[nWords];
	...
	template<typename T, size_t BitsPerItem> constexpr void copyFrom(Span<T> arr)
	{
		...
		size_t arrWords = size_t(arr.Length()) * BitsPerItem / (sizeof(basicType) * 8);
...
public:
	constexpr SBitset(decltype(null)=null): v{} {}

	template<typename T, typename=Requires<
		CIntegral<TRemoveConstRef<T>>
	>> constexpr SBitset(Span<T> arr): v{}
	{copyFrom<T, sizeof(T)*8>(arr);}

	constexpr SBitset(CSpan<bool> arr): v{}
	...
	*/
	constexpr SBitset(InitializerList<uint64> arr): v{}
	{copyFrom<const uint64, sizeof(uint64)*8>(arr);}

	//! Initialize only the first 64 bits.
	/*!
	  If N > 64 the remaining bits will be zero.
	  Use SBitset(x) when possible because the compiler will more likely optimize this rather than the initializer list
	  version above (SBitset{x}). Anyway there is no performance difference with -O2 or higher in GCC, Clang and MSVC.
	*/
	constexpr SBitset(uint64 x): v{basicType(x)} {}
	...
	*/
	template<typename Char> constexpr bool operator()(Index index) const {return size_t(index) < N && operator[](index);}
	...
	constexpr SBitset& operator=(decltype(null)) noexcept
	{
	...
	constexpr bool operator==(decltype(null)) const noexcept
	{
		basicType res = 0;
		for(size_t i = 0; i < nWords; i++)
			res |= v[i];
	...
	constexpr bool operator!=(decltype(null)) const noexcept {return !operator==(null);}
