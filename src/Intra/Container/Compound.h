...
public:
	...
	template<typename U> explicit(!CConvertibleTo<U&&, T>) constexpr Optional(U&& value): Optional(Construct, INTRA_FWD(value)) {}
	...
	constexpr ~Optional() {if(mHasValue) mVal.Value.~T();}
	...
	[[nodiscard]] INTRA_FORCEINLINE constexpr bool operator==(TUndefined) const {return !mHasValue;}
	...
	[[nodiscard]] INTRA_FORCEINLINE constexpr const T& Or(const T& fallback) const {return mHasValue? mVal.Value: fallback;}
...
private:
	union storage
	{
		T Value;
		char Bytes[1];
		constexpr storage(TUndefined) noexcept: Bytes{} {};
...
template<size_t I> constexpr auto At = []<class T>(T&& v) -> decltype(auto) requires CHasIndex<T> || z_D::CTupleElementDefined<T> || CHasReflectFieldPointersOf<T>
{
...
constexpr decltype(auto) ForEachFieldGenericHelper(F&& f, T&& obj, TIndexSeq<Is...>) {
	if constexpr((CCallable<F, decltype(At<Is>(obj)), TIndex<Is>> && ...))
	{
...
template<bool ReturnArray> constexpr auto ForEachFieldGeneric = []<typename F>(F&& f) {
	return [f = FunctorOf(INTRA_FWD(f))]<CStaticLengthContainer Object>(Object&& obj) mutable {
...
namespace z_D {
template<typename F, typename S, size_t... Is> constexpr decltype(auto) ForFieldAtRuntimeHelper(size_t index, [[maybe_unused]] F&& f, S&& t, TIndexSeq<Is...>)
{
...
constexpr auto ForFieldAtRuntime = []<typename F1>(Index index, F1&& f) {
	return [index, f = FunctorOf(INTRA_FWD(f))]<CStaticLengthContainer S>(S&& t) mutable -> decltype(auto) {
		INTRA_PRECONDITION(size_t(index) < StaticLength<S>);
		return z_D::ForFieldAtRuntimeHelper(size_t(index), f, INTRA_FWD(t), TMakeIndexSeq<StaticLength<S>>());
	};
};
...
template<size_t I, typename T> INTRA_FORCEINLINE constexpr decltype(auto) getStorage_(T&& vs) noexcept
{
...
template<typename... Ts> class Variant: private z_D::VariantStorage<Ts...>
{
	...
	static_assert(((CObject<Ts> && !CArrayType<Ts> && CDestructible<Ts>) && ...));
...
public:
	...
	// TODO: implement "sane variant constructor" (as in C++20)
	...
	constexpr Variant(T&& t) noexcept(CNothrowConstructible<InitType<T>, T>):
		Variant(ConstructAt<InitIndex<T>>, INTRA_FWD(t)) {}
	...
	constexpr explicit Variant(TConstructAt<I>, Args&&... args)
		...
		z_D::VariantStorage<Ts...>(TIndex<I>{}, INTRA_FWD(args)...),
		mAlternativeIndex(I) {}
...
constexpr auto VariantVisit = []<typename F>(F&& f) {
	return [f = FunctorOf(INTRA_FWD(f))](auto&& arg0, auto&&... args) {
		//TODO
