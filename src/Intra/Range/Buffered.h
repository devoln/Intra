...
RBuffered<TRangeOf<R>, Span<TValueTypeOfAs<R>>>> Buffered(R&& range, Span<TValueTypeOfAs<R>> buffer)
{return {ForwardAsRange<R>(range), buffer};}

template<typename R> [[nodiscard]] inline Requires<
	CAsAccessibleRange<R>,
RBuffered<TRangeOf<R>, Array<TValueTypeOfAs<R>>>> Buffered(R&& range, size_t bufferSize)
