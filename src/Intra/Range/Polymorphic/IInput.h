...
{
public:
	virtual index_t PopFirstCount(ClampedSize count)
	{
		for(size_t i = 0; i < size_t(count); i++)
		{
			if(this->Empty()) return index_t(i);
			this->PopFirst();
		}
		return index_t(count);
	}

	virtual index_t ReadWrite(Span<TRemoveConstRef<T>>& dst) = 0;
};

using IInputStream = IInputEx<char>;
INTRA_END
