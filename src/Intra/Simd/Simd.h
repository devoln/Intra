...
{
	[[nodiscard]] INTRA_FORCEINLINE float4 INTRA_VECTORCALL operator()(float4 a, float4 b) const {return Simd::Min(a, b);}
};
template<> struct TMax::Typed<const float4&, const float4&>
{
	[[nodiscard]] INTRA_FORCEINLINE float4 INTRA_VECTORCALL operator()(float4 a, float4 b) const {return Simd::Max(a, b);}
};
#endif
#ifdef INTRA_SIMD_INT4_SUPPORT
template<> struct TMin::Typed<const int4&, const int4&>
{
	[[nodiscard]] INTRA_FORCEINLINE float4 INTRA_VECTORCALL operator()(int4 a, int4 b) const {return Simd::Min(a, b);}
};
template<> struct TMax::Typed<const int4&, const int4&>
{
	[[nodiscard]] INTRA_FORCEINLINE float4 INTRA_VECTORCALL operator()(int4 a, int4 b) const {return Simd::Max(a, b);}
};
#endif
#ifdef INTRA_SIMD_FLOAT8_SUPPORT
template<> struct TMin::Typed<const float8&, const float8&>
{
	[[nodiscard]] INTRA_FORCEINLINE float8 INTRA_VECTORCALL operator()(float8 a, float8 b) const {return Simd::Min(a, b);}
};
template<> struct TMax::Typed<const float8&, const float8&>
{
	[[nodiscard]] INTRA_FORCEINLINE float8 INTRA_VECTORCALL operator()(float8 a, float8 b) const {return Simd::Max(a, b);}
};
#endif
#ifdef INTRA_SIMD_INT8_SUPPORT
template<> struct TMin::Typed<const int8&, const int8&>
{
	[[nodiscard]] INTRA_FORCEINLINE int8 INTRA_VECTORCALL operator()(int8 a, int8 b) const {return Simd::Min(a, b);}
};
template<> struct TMax::Typed<const int8&, const int8&>
{
	[[nodiscard]] INTRA_FORCEINLINE int8 INTRA_VECTORCALL operator()(int8 a, int8 b) const {return Simd::Max(a, b);}
};
#endif
...
#ifdef INTRA_SIMD_FLOAT4_SUPPORT
[[nodiscard]] constexpr index_t LengthOf(Simd::float4) {return 4;}
#endif
#ifdef INTRA_SIMD_INT4_SUPPORT
[[nodiscard]] constexpr index_t LengthOf(Simd::int4) {return 4;}
#endif
#ifdef INTRA_SIMD_FLOAT8_SUPPORT
[[nodiscard]] constexpr index_t LengthOf(Simd::float8) {return 8;}
#endif
#ifdef INTRA_SIMD_INT8_SUPPORT
[[nodiscard]] constexpr index_t LengthOf(Simd::int8) {return 8;}
#endif

template<typename T> T Load(const TScalarOf<T>* src);
template<typename T> T LoadAligned(const TScalarOf<T>* src);
#ifdef INTRA_SIMD_FLOAT4_SUPPORT
template<> [[nodiscard]] INTRA_FORCEINLINE float4 Load(const float* src) {return Load4(src);}
template<> [[nodiscard]] INTRA_FORCEINLINE float4 LoadAligned(const float* src) {return LoadAligned4(src);}
#endif
#ifdef INTRA_SIMD_INT4_SUPPORT
template<> [[nodiscard]] INTRA_FORCEINLINE int4 Load(const int* src) {return Load4(src);}
template<> [[nodiscard]] INTRA_FORCEINLINE int4 LoadAligned(const int* src) {return LoadAligned4(src);}
#endif
#ifdef INTRA_SIMD_FLOAT8_SUPPORT
template<> [[nodiscard]] INTRA_FORCEINLINE float8 Load(const float* src) {return Load8(src);}
template<> [[nodiscard]] INTRA_FORCEINLINE float8 LoadAligned(const float* src) {return LoadAligned8(src);}
#endif
#ifdef INTRA_SIMD_INT8_SUPPORT
template<> [[nodiscard]] INTRA_FORCEINLINE int8 Load(const int* src) {return Load8(src);}
template<> [[nodiscard]] INTRA_FORCEINLINE int8 LoadAligned(const int* src) {return LoadAligned8(src);}
#endif

template<typename T, typename = Requires<CFloatingPoint<TScalarOf<T>>>>
[[nodiscard]] INTRA_FORCEINLINE T INTRA_VECTORCALL Truncate(T x) noexcept
{return CastToFloat(TruncateToInt(x));}

template<typename T, typename = Requires<CFloatingPoint<TScalarOf<T>>>>
[[nodiscard]] INTRA_FORCEINLINE T INTRA_VECTORCALL Round(T a)
{
...
[[nodiscard]] INTRA_FORCEINLINE float4 INTRA_VECTORCALL Floor(float4 x) {return _mm_floor_ps(x);}
[[nodiscard]] INTRA_FORCEINLINE float4 INTRA_VECTORCALL Ceil(float4 x) {return _mm_ceil_ps(x);}
#endif

#ifdef INTRA_SIMD_INT4_SUPPORT

#if INTRA_SIMD_LEVEL >= INTRA_SIMD_LEVEL_SSE2
[[nodiscard]] INTRA_FORCEINLINE int4 INTRA_VECTORCALL RoundToInt(float4 x) noexcept
{
...
#if INTRA_SIMD_LEVEL < INTRA_SIMD_LEVEL_SSE4_1
[[nodiscard]] INTRA_FORCEINLINE float4 INTRA_VECTORCALL Floor(float4 x)
{
...
[[nodiscard]] INTRA_FORCEINLINE float4 INTRA_VECTORCALL Ceil(float4 x)
{
	const float4 fi = Truncate(x);
	const int4 igx = fi < x;
	const float4 j = float4(igx & int4(Set<float4>(1)));
...
[[nodiscard]] INTRA_FORCEINLINE float8 INTRA_VECTORCALL Floor(float8 x) {return _mm256_floor_ps(x);}
[[nodiscard]] INTRA_FORCEINLINE float8 INTRA_VECTORCALL Ceil(float8 x) {return _mm256_ceil_ps(x);}
...
[[nodiscard]] INTRA_FORCEINLINE int8 INTRA_VECTORCALL RoundToInt(float8 x) noexcept
{
	return int8(_mm256_cvtps_epi32(x));
}
#else
[[nodiscard]] INTRA_FORCEINLINE int8 INTRA_VECTORCALL RoundToInt(float8 x) noexcept
{
...
[[nodiscard]] INTRA_FORCEINLINE float8 INTRA_VECTORCALL Floor(float8 x)
{
...
[[nodiscard]] INTRA_FORCEINLINE float8 INTRA_VECTORCALL Ceil(float8 x)
{
	const float8 fi = Truncate(x);
	const int8 igx = fi < x;
	const float8 j = float8(igx & int8(Set<float8>(1)));
...
//a + b*c
[[nodiscard]] INTRA_FORCEINLINE float4 INTRA_VECTORCALL MultiplyAccumulate(float4 a, float4 b, float4 c)
{
	return _mm_fmadd_ps(b, c, a);
}

[[nodiscard]] INTRA_FORCEINLINE float8 INTRA_VECTORCALL MultiplyAccumulate(float8 a, float8 b, float8 c)
{
	return _mm256_fmadd_ps(b, c, a);
}

[[nodiscard]] INTRA_FORCEINLINE float4 INTRA_VECTORCALL MultiplyAccumulate(float a, float4 b, float4 c)
{
	return MultiplyAccumulate(Set<float4>(a), b, c);
}

[[nodiscard]] INTRA_FORCEINLINE float8 INTRA_VECTORCALL MultiplyAccumulate(float a, float8 b, float8 c)
{
	return MultiplyAccumulate(Set<float8>(a), b, c);
}

[[nodiscard]] INTRA_FORCEINLINE float4 INTRA_VECTORCALL MultiplyAccumulate(float4 a, float4 b, float c)
{
	return MultiplyAccumulate(a, b, Set<float4>(c));
}

[[nodiscard]] INTRA_FORCEINLINE float8 INTRA_VECTORCALL MultiplyAccumulate(float8 a, float8 b, float c)
{
	return MultiplyAccumulate(a, b, Set<float8>(c));
}

[[nodiscard]] INTRA_FORCEINLINE float4 INTRA_VECTORCALL MultiplyAccumulate(float a, float4 b, float c)
{
	return MultiplyAccumulate(Set<float4>(a), b, Set<float4>(c));
}

[[nodiscard]] INTRA_FORCEINLINE float8 INTRA_VECTORCALL MultiplyAccumulate(float a, float8 b, float c)
{
	return MultiplyAccumulate(Set<float8>(a), b, Set<float8>(c));
}

#else

template<typename T1, typename T2, typename T3>
[[nodiscard]] INTRA_FORCEINLINE auto INTRA_VECTORCALL MultiplyAccumulate(T1 a, T2 b, T3 c) {return a + b*c;}

#endif

template<typename T, typename = Requires<CFloatingPoint<TScalarOf<T>>>>
[[nodiscard]] INTRA_FORCEINLINE T INTRA_VECTORCALL Fract(T x) {return x - Floor(x);}

template<typename T, typename = Requires<CFloatingPoint<TScalarOf<T>>>>
[[nodiscard]] INTRA_FORCEINLINE T INTRA_VECTORCALL Mod(T a, T aDiv) {return a - Floor(a / aDiv) * aDiv;}

template<typename T, typename = Requires<CSame<TScalarOf<T>, float>>>
[[nodiscard]] INTRA_FORCEINLINE T INTRA_VECTORCALL ModSigned(T a, T aDiv) {return a - Truncate(a / aDiv) * aDiv;}

template<typename T, typename = Requires<CSame<TScalarOf<T>, float>>>
[[nodiscard]] INTRA_FORCEINLINE T INTRA_VECTORCALL Abs(T v) noexcept {return T(IntAnalogOf<T>(v) & 0x7FFFFFFF);}
...
{
	static INTRA_FORCEINLINE T INTRA_VECTORCALL Calc(T m) noexcept
	{
...
template<int Order, typename T, typename = Requires<CSame<TScalarOf<T>, float>>>
[[nodiscard]] INTRA_FORCEINLINE T INTRA_VECTORCALL Log2Order(T x)
{
	T one = Set<T>(1);
...
template<int Order, typename T, typename = Requires<CSame<TScalarOf<T>, float>>>
[[nodiscard]] INTRA_FORCEINLINE T INTRA_VECTORCALL LogOrder(T x)
{
	return Log2Order<Order>(x) / float(1.442695040888963407359924681001892137426645954153);
}

template<typename T, typename = Requires<CSame<TScalarOf<T>, float>>>
[[nodiscard]] INTRA_FORCEINLINE T INTRA_VECTORCALL Log2(T x) {return Log2Order<5>(x);}

template<typename T, typename = Requires<CSame<TScalarOf<T>, float>>>
[[nodiscard]] inline T INTRA_VECTORCALL Log(T x) {return LogOrder<5>(x);}
