...
template<typename T> INTRA_FORCEINLINE T AtomicBase<T>::Get ## mo() const noexcept \
{return __atomic_load_n(&mValue, __ATOMIC_ ## MO);}

#define INTRA_ATOMIC_METHOD_SET(mo, MO) \
template<typename T> INTRA_FORCEINLINE void AtomicBase<T>::Set ## mo(T val) noexcept \
{__atomic_store_n(&mValue, val, __ATOMIC_ ## MO);}

#define INTRA_ATOMIC_METHOD_GET_SET(mo, MO) \
template<typename T> INTRA_FORCEINLINE T AtomicBase<T>::GetSet ## mo(T val) noexcept \
{return __atomic_exchange_n(&mValue, val, __ATOMIC_ ## MO);}

#define INTRA_ATOMIC_METHOD_CAS(method, MO, weak) \
template<typename T> INTRA_FORCEINLINE bool AtomicBase<T>::method(T expected, T desired) noexcept \
...
template<typename T> INTRA_FORCEINLINE T AtomicInteger<T>::method ## mo(T rhs) noexcept \
...
template<typename T> INTRA_FORCEINLINE T AtomicInteger<T>::Increment ## mo() noexcept {return Add ## mo a(1);}\
template<typename T> INTRA_FORCEINLINE T AtomicInteger<T>::Decrement ## mo() noexcept {return Sub ## mo a(1);}\
template<typename T> INTRA_FORCEINLINE T AtomicInteger<T>::GetIncrement ## mo() noexcept {return GetAdd ## mo a(1);}\
template<typename T> INTRA_FORCEINLINE T AtomicInteger<T>::GetDecrement ## mo() noexcept {return GetSub ## mo a(1);}
