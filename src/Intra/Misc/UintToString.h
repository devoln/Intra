...
constexpr index_t UintToString100(unsigned x, char* dst)
{
	...
	{
		unsigned low = x;
		unsigned ll = ((uint32(low) * 103) >> 9) & 0x1E;
		low += ll * 3;
		ll = ((low & 0xF0) >> 4) | ((low & 0x0F) << 8);
		ll |= 0x3030;
		Misc::BinarySerializeLE(uint16(ll), dst);
		return 2;
	}
	return 0;
}
...
constexpr index_t UintToString10000(unsigned x, char* dst)
{
	...
	{
		Misc::BinarySerializeLE(uint16((ll >> 8) & 0xFFFF), dst);
...
constexpr index_t UintToString100m(uint32 x, char* dst, index_t digits)
{
	...
	if(d >= 2)
	{
		Misc::BinarySerializeLE(uint16(ll & 0xFFFF), dst);
		dst += 2;
		ll >>= 16;
		d -= 2;
	}
...
constexpr index_t UintToString(uint32 x, char* dst)
{
	...
	else
	{
		...
		const unsigned high = unsigned((uint64(x) * 0x55E63B89) >> 57);
