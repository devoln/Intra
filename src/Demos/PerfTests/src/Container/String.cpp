...
template<typename String> double TestStringReading(unsigned times, size_t strsize)
{
	int c=0;
	...
	for(unsigned j=0; j<times; j++)
	{
		for(size_t i=0; i<strsize; i++)
			c += str[i];
		srand(unsigned(c)); //Препятствуем оптимизации, удаляющей внешний цикл
	}
	return timer.ElapsedSeconds();
}


template<typename String> double TestStringWriting(unsigned times, size_t strsize)
{
	char c = 'd';
	String str(strsize, ' ');
...
template<typename String> double TestStringComparing1(unsigned times, size_t strsize)
{
...
template<typename String> double TestStringComparing2(unsigned times, size_t strsize)
{
	//bool(*func)(const String& str1, const String& str2)=cmpString<String>;
...
template<typename String> double TestStringComparing3(unsigned times, size_t strsize)
{
	//bool(*func)(const String& str1, const String& str2)=cmpString<String>;
	String str1 = GenerateRandomString<String>(strsize);
	String str2 = str1;
...
template<typename String> double TestStringAccumulation1(unsigned times, size_t strsize, unsigned concatenations)
{
	...
	{
		String ss;
		for(unsigned j=0; j<concatenations; j++)
			ss += c;
	}
	return timer.ElapsedSeconds();
}

template<typename S> double TestStringAccumulation2(unsigned times, unsigned concatenations)
{
	Stopwatch timer;
	for(unsigned i=0; i<times; i++)
	{
...
template<typename S> double TestStringConcatenationChain(unsigned times, size_t strsize)
{
...
template<typename S> double TestStringConcatenationOptimizedChain(unsigned times, size_t strsize)
{
	S c = GenerateRandomString<S>(strsize);
	Stopwatch timer;
	for(unsigned j=0; j<times; j++)
...
template<typename S> double TestStringConcatenation(unsigned times, size_t strsize)
{
...
template<typename S> double TestStringAddChar(unsigned times)
{
	S ss;
	Stopwatch timer;
	for(unsigned j=0; j<times; j++)
...
template<typename S> double TestStringAddChar2(unsigned times, unsigned concatenations)
{
	Stopwatch timer;
	for(unsigned i=0; i<times; i++)
	{
...
template<typename S> double TestStringPlusChar(unsigned times, size_t strsize)
{
	S c = GenerateRandomString<S>(strsize);
	Stopwatch timer;
	for(unsigned j=0; j<times; j++)
...
template<typename S> double TestStringCopying(unsigned times, size_t strsize)
{
...
template<typename S> double TestCStrToStringCopying(unsigned times, size_t strsize)
{
...
double TestSprintf(unsigned times)
{
	Stopwatch timer;
	char str[40];
	for(unsigned i=0; i<times; i++)
...
#ifndef INTRA_MINIMIZE_CRT
double TestStdToStringFormatting(unsigned times)
{
	Stopwatch timer;
	for(unsigned i=0; i<times; i++)
	{
...
double TestStrStreamFormatting(unsigned times)
{
	Stopwatch timer;
	for(unsigned i=0; i<times; i++)
	{
...
double TestSharedStrStreamFormatting(unsigned times)
{
	Stopwatch timer;
	std::stringstream ss;
	for(unsigned i = 0; i<times; i++)
...
template<typename S> double TestStringFormatting(unsigned times)
{
	Stopwatch timer;
	for(unsigned i=0; i<times; i++)
...
double TestStackStreamFormatting(unsigned times)
{
	char stackBuffer[40];
	Stopwatch timer;
	for(unsigned i=0; i<times; i++)
...
template<typename S> double TestAppenderStreamFormatting(unsigned times)
{
	Stopwatch timer;
	for(unsigned i = 0; i<times; i++)
	{
...
template<typename S> double TestStringSubStr(unsigned times)
{
...
double TestStringSubStrView(unsigned times)
{
