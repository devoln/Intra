...
{
	unsigned rndCounter = 1;
public:
	using BasicFormatter::PushFont;
	using BasicFormatter::PopFont;

	void BeginCode(IOutputStream& s) override {s << "\n<pre>\n";}
	void EndCode(IOutputStream& s) override {s << "\n</pre>\n";}

	void PushStyle(IOutputStream& s, StringView style) override {s << "<span class='" << style << "'>";}
	void PopStyle(IOutputStream& s) override {s << "</span>";}

	void HorLine(IOutputStream& s) override {s << "<hr>";}

	void LineBreak(IOutputStream& s) override {s << "<br />\n";}

	void PrintPreformatted(IOutputStream& s, StringView text) override
	{
	...
	{
		const unsigned rndSeed = (unsigned(reinterpret_cast<size_t>(label.Data())) ^ ToHash(label)) + rndCounter;
		const uint64 id = FastUniform<uint64>(rndSeed)();
		s << *String::Format(HtmlWriter_SpoilerBeginCode)(id)(id)(label);
	}

	void endSpoiler(IOutputStream& s) override {s << HtmlWriter_SpoilerEndCode;}
};
