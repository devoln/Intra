...
struct FormatInfo
{
	uint16 Type: 2; //0 - Invalid, 1 - normalized, 2 - floating point, 3 - integer
	uint16 IsSigned: 1;
	uint16 IsPacked: 1;
	uint16 IsBasic: 1;
	uint16 IsCompressed: 1;
	uint16 HasLuminance: 1;
	uint16 HasAlpha: 1;
	uint16 HasDepth: 1;
	uint16 HasStencil: 1;
	uint16 ComponentCount: 2; //Component number - 1 (to fit into 2 bits)
	uint16 HasSharedExponent: 1;
	uint16 Is_sRGB: 1;
...
ImageFormat ImageFormat::FromString(StringView str)
{
	...
	if(formatTable == null)
	{
		...
		for(uint16 i = ImageFormat::FirstOfCompressed; i<ImageFormat::EndOfCompressed; i++)
			formatTable[ImageFormat(i).ToString()] = ImageFormat(i);
		for(uint16 i = ImageFormat::FirstOfBasic; i<ImageFormat::EndOfBasic; i++)
			formatTable[ImageFormat(i).ToString()] = ImageFormat(i);
	}
