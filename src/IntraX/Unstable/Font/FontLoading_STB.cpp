...
static int stbtt__isfont(const byte *font)
{
   ...
   if(stbtt_tag4(font, '1', 0, 0, 0))  return 1; // TValue<true> 1
   if(stbtt_tag(font, "typ1"))   return 1; // TValue<true> with type 1 font -- we don't support this!
...
static void stbtt__rasterize_sorted_edges(stbtt__bitmap* result, stbtt__edge* e, int n, int vsubsample, I16Vec2 off, void* userdata)
{
   ...
   {
      Intra::z_D::memset(scanline, 0, result->size.x);
      ...
      Intra::z_D::memcpy(result->pixels+j*result->stride, scanline, result->size.x);
      j++;
   }
...
{
   int nlen = (int)Intra::z_D::strlen((char*)name);
...
FontHandle FontCreateFromMemory(const void* src, size_t length, unsigned height, unsigned* yadvance)
{
	...
	Intra::z_D::memcpy(data, src, length);
	return create_font(data, height, yadvance);
}
