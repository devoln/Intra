...
{
	index_t targetIndex = 0;
	const auto len = real.Length();
...
{
	float nextSine = 0;
	for(index_t i = 1; i < real.Length(); i <<= 1)
	{
		const auto next = i << 1;
		...
		for(index_t j = 0; j < i; j++) // iterations through groups with different transform factors
		{
			for(index_t k = j; k < real.Length(); k += next) // iterations through pairs within group
			{
				const index_t match = k + i;
...
bool InplaceInverseFFT(Span<cfloat> data)
{
	...
	rearrangeData(data);
	makeTransform(data, Constants.PI);
	Multiply(data.ReinterpretUnsafe<float>(), 1.0f/data.Length());
}
...
void InplaceFFT(Span<float> real, Span<float> imag)
{
	...
	INTRA_PRECONDITION(IsPow2(size_t(real.Length())));
