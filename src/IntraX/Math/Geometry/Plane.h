...
template<typename T> struct Plane
{
	...
	constexpr Plane(const Vector3<T>& planeNormal, const T& distanceFromZero) noexcept:
		Normal(planeNormal), D(distanceFromZero) {}

	explicit constexpr Plane(const Vector4<T>& asVec4) noexcept: AsVec4(asVec4) {}

	constexpr Plane(const T& a, const T& b, const T& c, const T& d) noexcept: AsVec4(a, b, c, d) {}
...
template<typename T> constexpr SignedDistanceNormalized(const Plane<T>& plane, const Vector3<T>& pt)
{return -Dot(pt, plane.Normal);}

template<typename T> INTRA_MATH_CONSTEXPR INTRA_FORCEINLINE SignedDistance(const Plane<T>& plane, const Vector3<T>& pt)
{return DistanceNormalized(pt, plane)/Length(plane.Normal);}

template<typename T> INTRA_MATH_CONSTEXPR INTRA_FORCEINLINE Distance(const Plane<T>& plane, const Vector3<T>& pt)
