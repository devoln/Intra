...
template<typename T> struct Quaternion
{
	...
	INTRA_FORCEINLINE Quaternion() = default;
	constexpr Quaternion(T X, T Y=0, T Z=0, T W=1): x(X), y(Y), z(Z), w(W) {}
	constexpr Quaternion(const Vector3<T>& v): xyz(v), w(1) {}
	...
	INTRA_FORCEINLINE Quaternion operator*(T rhs) const
	{return {x*rhs, y*rhs, z*rhs, w*rhs};}

	INTRA_FORCEINLINE Quaternion operator/(T rhs) const
	{return {x/rhs, y/rhs, z/rhs, w/rhs};}

	INTRA_FORCEINLINE Quaternion operator+(const Quaternion& rhs) const
	...
	INTRA_FORCEINLINE Quaternion& operator+=(const Quaternion& q) {xyzw += q.xyzw; return *this;}
	INTRA_FORCEINLINE Quaternion& operator-=(const Quaternion& q) {xyzw -= q.xyzw; return *this;}
	INTRA_FORCEINLINE Quaternion& operator*=(T rhs) {xyzw *= rhs; return *this;}
	INTRA_FORCEINLINE Quaternion& operator/=(T rhs) {xyzw /= rhs; return *this;}
	INTRA_FORCEINLINE Quaternion& operator*=(const Quaternion& q) {return *this = *this*q;}
	...
	constexpr bool operator==(const Quaternion& rhs) const {return xyzw == rhs.xyzw;}
	constexpr bool operator!=(const Quaternion& rhs) const {return !operator==(rhs);}

	static INTRA_MATH_CONSTEXPR Quaternion RotationEulerRad(T yaw, T pitch, T roll)
	{
...
template<typename T> INTRA_FORCEINLINE T Length(const Quaternion<T>& q) {return Length(q.xyzw);}
template<typename T> INTRA_FORCEINLINE T LengthSqr(const Quaternion<T>& q) {return LengthSqr(q.xyzw);}
template<typename T> INTRA_FORCEINLINE Quaternion<T> Normalize(const Quaternion<T>& q) {return Normalize(q.xyzw);}
template<typename T> INTRA_FORCEINLINE Quaternion<T> Conjugate(const Quaternion<T>& q) {return Quaternion<T>(-q.x, -q.y, -q.z, q.w);}
...
template<typename T> Quaternion<T>::Quaternion(const Matrix3<T>& mat)
{
	...
	unsigned i = unsigned(mat[1][1] > mat[0][0]);
