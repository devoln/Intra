...
template<typename T> inline void Destruct(Span<T> dst)
{
	if constexpr(!CTriviallyDestructible<T>)
		for(auto& x: dst) x.~T();
	else if constexpr(DebugCheckLevel >= 1)
		Misc::CImp::memset(dst.Data(), 0xDE, size_t(dst.Length())*sizeof(dst.First()));
}
...
template<typename T> void CopyRecreate(Span<T> dst, CSpan<T> src)
{
	...
	if constexpr(CTriviallyCopyable<T>) BitwiseCopyUnsafe(dst, src);
...
template<typename T, typename U> void CopyInit(Span<T> dst, CSpan<U> src)
{
	...
	if constexpr(CTriviallyCopyable<T>)
		Misc::BitwiseCopyUnsafe(dst.Data(), src.Data(), dst.Length());
	else for(auto& dstVal: dst)
		new(Construct, &dstVal) T(src.Next());
}
...
template<typename T> void MoveInitBackwards(Span<T> dst, Span<T> src)
{
	INTRA_PRECONDITION(dst.Length() == src.Length());
	if constexpr(CTriviallyCopyable<T>)
		Misc::BitwiseCopyBackwardsUnsafe(dst.Data(), src.Data(), dst.Length());
	else while(!dst.Empty())
	{
		new(Construct, &dst.Last()) T(Move(src.Last()));
		dst.PopLast();
		src.PopLast();
	}
}
...
template<typename T> void MoveInitDelete(Span<T> dst, Span<T> src)
{
	...
	{
		new(Construct, &dst.Next()) T(Move(srcVal));
		srcVal.~T();
	}
}
...
template<typename T> void MoveInitDeleteBackwards(Span<T> dst, Span<T> src)
{
	INTRA_PRECONDITION(dst.Length() == src.Length());
	if constexpr(CTriviallyRelocatable<T>)
		BitwiseCopyBackwardsUnsafe(DataOf(dst), DataOf(src), LengthOf(dst));
	else while(!dst.Empty())
	{
		new(Construct, &dst.Last()) T(Move(src.Last()));
		src.Last().~T();
...
template<typename T> void MoveAssignDelete(Span<T> dst, Span<T> src)
{
	INTRA_PRECONDITION(dst.Length() == src.Length());
	if constexpr(CTriviallyRelocatable<T>)
		BitwiseCopyUnsafe(dst, src);
	else for(auto& srcVal: src)
	{
		dst.Next() = Move(srcVal);
		srcVal.~T();
	}
}
