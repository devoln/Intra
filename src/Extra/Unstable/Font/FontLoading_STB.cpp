...
#include "Extra/Unstable/Font/FontLoading.h"
...
using Intra::Sqrt;
using Intra::Ceil;
using Intra::BitwiseCopyUnsafe;
using Intra::GlobalHeap;
...
const uint16 STBTT_MACSTYLE_DONTCARE=0, STBTT_MACSTYLE_BOLD=1, STBTT_MACSTYLE_ITALIC=2, STBTT_MACSTYLE_UNDERSCORE=4, STBTT_MACSTYLE_NONE=8;
...
#define ttFixed(p)    ttLONG(p)
   ...
   #define ttUSHORT(p)   (*(uint16*)(p))
   #define ttSHORT(p)    (*(short*)(p))
   #define ttULONG(p)    (*(unsigned*)(p))
   #define ttLONG(p)     (*(int*)(p))

#else

   uint16 ttUSHORT(const byte *p) {return uint16((p[0] << 8)+p[1]);}
   short ttSHORT(const byte *p) {return short((p[0] << 8)+p[1]);}
   unsigned ttULONG(const byte *p) {return unsigned((p[0] << 24)+(p[1] << 16)+(p[2] << 8)+p[3]);}
...
static int stbtt__isfont(const byte *font)
{
   ...
   if(stbtt_tag4(font, '1', 0, 0, 0))  return 1; // TBool<true> 1
   if(stbtt_tag(font, "typ1"))   return 1; // TBool<true> with type 1 font -- we don't support this!
...
static unsigned stbtt__find_table(byte *data, unsigned fontstart, const char *tag)
{
   int num_tables=ttUSHORT(data+fontstart+4);
   unsigned tabledir=fontstart+12;
...
int stbtt_InitFont(stbtt_fontinfo* info, const byte* data2, int fontstart)
{
   ...
   {
      unsigned encoding_record=cmap+4 + 8*i;
...
int stbtt_FindGlyphIndex(const stbtt_fontinfo* info, int unicode_codepoint)
{
   ...
   int index_map = info->index_map;
	...
	uint16 format=ttUSHORT(data+index_map);
	if(format==0)
	{
	  // apple byte encoding
	...
	{
		unsigned first=ttUSHORT(data+index_map+6);
		unsigned count=ttUSHORT(data+index_map+8);
		if((unsigned)unicode_codepoint>=first && (unsigned)unicode_codepoint<first+count)
	...
	else if(format==4)
	{
		...
		uint16 segcount=ttUSHORT(data+index_map+6)/2;
	    uint16 searchRange=ttUSHORT(data+index_map+8)/2;
	    uint16 entrySelector=ttUSHORT(data+index_map+10);
	    uint16 rangeShift=ttUSHORT(data+index_map+12)/2;

		// do a binary search of the segments
		unsigned endCount = unsigned(index_map+14);
		...
		uint16 item = uint16((search-endCount)/2);
		...
		uint16 offset=ttUSHORT(data+index_map+14+segcount*6+2+2*item);
		if(offset==0) return (uint16)(unicode_codepoint+ttSHORT(data+index_map+14+segcount*4+2+2*item));

		return ttUSHORT(data+offset+(unicode_codepoint-start)*2+index_map+14+segcount*6+2+2*item);
	}
	else if(format==12 || format==13)
	{
      unsigned ngroups = ttULONG(data+index_map+12);
      ...
      while(low<high)
	  {
         ...
         unsigned start_char = ttULONG(data+index_map+16+mid*12);
         unsigned end_char = ttULONG(data+index_map+16+mid*12+4);
         if(unsigned(unicode_codepoint)<start_char) high=mid;
         else if(unsigned(unicode_codepoint)>end_char) low=mid+1;
         else
		 {
            unsigned start_glyph = ttULONG(data+index_map+16+mid*12+8);
	...
	else if(numberOfContours==-1)
	{
		...
		while(more)
		{
			...
			{ // WE_HAVE_AN_X_AND_YSCALE
				...
				for(uint16 j=0; j<4; j++) mtx[j]=ttSHORT(comp)/16384.0f, comp+=2;
			...
			if(comp_num_verts>0)
			{
				...
				BitwiseCopyUnsafe<stbtt_vertex>({tmp, size_t(num_vertices)}, {vertices, size_t(num_vertices)});
				BitwiseCopyUnsafe<stbtt_vertex>({tmp+num_vertices, size_t(comp_num_verts)}, {comp_verts, size_t(comp_num_verts)});
				delete[] vertices;
				vertices=tmp;
				delete[] comp_verts;
...
int stbtt_GetGlyphKernAdvance(const stbtt_fontinfo* info, int glyph1, int glyph2)
{
   ...
   unsigned needle=(glyph1<<16)|glyph2;
   while(l<=r)
   {
      int m=(l+r)/2;
      const unsigned straw=ttULONG(data+18+(m*6)); // note: unaligned read
...
static void stbtt__rasterize_sorted_edges(stbtt__bitmap* result, stbtt__edge* e, int n, int vsubsample, SVec2 off, void* userdata)
{
   ...
   {
      Intra::CImp::memset(scanline, 0, result->size.x);
      ...
      Intra::CImp::memcpy(result->pixels+j*result->stride, scanline, result->size.x);
      j++;
   }
...
static void stbtt__rasterize(stbtt__bitmap* result, Vec2* pts, int* wcount, int windings, Vec2 scale, Vec2 shift, SVec2 off, int invert, void* userdata)
{
   ...
   stbtt__rasterize_sorted_edges(result, &e[0], (unsigned)e.Count(), vsubsample, off, userdata);
}
...
static bool stbtt__matches(byte* fc, unsigned offset, byte* name, int flags)
{
   int nlen = (int)Intra::CImp::strlen((char*)name);
   if(!stbtt__isfont(fc+offset)) return 0;

   // check italics/bold/underline flags in macStyle...
   if(flags)
   {
      const unsigned hd=stbtt__find_table(fc, offset, "head");
      if((ttUSHORT(fc+hd+44)&7)!=(flags&7)) return false;
   }

   const unsigned nm=stbtt__find_table(fc, offset, "name");
...
FontHandle FontCreateFromMemory(const void* src, size_t length, unsigned height, unsigned* yadvance)
{
	byte* data = GlobalHeap.Allocate(length, INTRA_SOURCE_INFO);
	Intra::CImp::memcpy(data, src, length);
	return create_font(data, height, yadvance);
}
