...
#include "Extra/Unstable/Image/Loaders/LoaderPlatform.h"
...
AnyImage LoadWithPlatform(IInputStream& stream)
{
	...
	result.SwapRB = false;
	const uint16 glformat = uint16(ilGetInteger(IL_IMAGE_FORMAT));
	switch(glformat)
	{
	...
	Misc::BitwiseCopyUnsafe(result.Data.First, ilGetData(), result.Data.Length());
