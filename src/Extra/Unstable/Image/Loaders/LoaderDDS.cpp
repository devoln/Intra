...
#include "Extra/Unstable/Image/Loaders/LoaderDDS.h"
#include "Extra/Unstable/Image/AnyImage.h"
#include "Extra/Unstable/Image/Bindings/DXGI_Formats.h"
...
ImageInfo LoaderDDS::GetInfo(IInputStream& stream) const
{
	...
	ImageInfo result;
	result.Size = {hdr.width, hdr.height, FMax(uint16(1), uint16(hdr.depth))};
	...
	if(hdr.flags & DDSD_MIPMAPCOUNT)
		result.MipmapCount = FMax(short(1), short(hdr.mipMapCount));
...
AnyImage LoaderDDS::Load(IInputStream& stream) const
{
	...
	bool swapRB;
	ImageInfo iInfo;
	iInfo.Size = {header.width, header.height, FMax(1u, header.depth)};
	...
	iInfo.MipmapCount = 0;
	if(header.flags & DDSD_MIPMAPCOUNT) iInfo.MipmapCount = FMax(short(1), short(header.mipMapCount));
