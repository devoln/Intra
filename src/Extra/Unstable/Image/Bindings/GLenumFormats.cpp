#include "Extra/Unstable/Image/Bindings/GLenumFormats.h"
...
uint16 ImageFormatToGLInternal(ImageFormat format, bool useSwizzling)
{
	...
	static const uint16 uncompressedTable[] = {
		0,
	...
	static const uint16 compressedTable[] = {
		//DXT
	...
	static const uint16 basicFormatTable[] = //Пользователю неважно, какой именно будет формат из соответствующих указанному базовому, поэтому реализация выбирает автоматически
	{
...
uint16 ImageFormatToGLExternal(ImageFormat format, bool swapRB, bool useSwizzling)
{
	...
	uint16 result = basic? basicFormatTable[format.value - ImageFormat::FirstOfBasic]: uncompressedTable[format.value];
	if(useSwizzling)
	{
...
uint16 ImageFormatToGLType(ImageFormat format)
{
	if(!format.IsValid()) return 0;

	static const uint16 uncompressedFormatTable[] = {
		0,
	...
	static const uint16 basicFormatTable[] = {
...
uint16 ImageTypeToGLTarget(ImageType type)
{
	static const uint16 imageTypeConvertTable[] = {
...
uint16 CubeFaceToGLTarget(CubeFace cf) {return uint16(GL::TEXTURE_CUBE_MAP_POSITIVE_X + byte(cf));}

ImageType GLTargetToImageType(uint16 gl_Target)
{
	for(uint16 i = 0; i < ImageType_End; i++)
