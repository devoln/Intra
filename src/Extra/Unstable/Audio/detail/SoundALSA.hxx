...
struct Buffer
{
	Buffer(unsigned buf, unsigned sample_count, unsigned sample_rate, unsigned ch, uint16 format):
		buffer(buf), sampleCount(sample_count), sampleRate(sample_rate), channels(ch), alformat(format) {}

	unsigned SizeInBytes() const {return unsigned(sampleCount*channels*sizeof(short));}
...
struct Instance
{
	Instance(unsigned src, BufferHandle my_parent): source(src), parent(my_parent) {}
...
{
	//StreamedBuffer(unsigned bufs[2], StreamingCallback callback, unsigned sample_count, unsigned sample_rate, uint16 ch):
	...
	unsigned buffers[2];
	unsigned source;
	unsigned sampleCount; //Размер половины буфера
	unsigned sampleRate;
	unsigned channels;
...
struct Context
{
	...
	void Prepare()
	{
		...
		unsigned exactSampleRate = SW_OUTPUT_DEVICE_SAMPLE_RATE;
...
BufferHandle BufferCreate(size_t sampleCount, unsigned channels, unsigned sampleRate)
{
	context.Prepare();
	unsigned buffer;
	alGenBuffers(1, &buffer);
	return new Buffer(buffer, unsigned(sampleCount), sampleRate, channels, get_format(channels));
}
...
StreamedBufferHandle StreamedBufferCreate(size_t sampleCount,
	unsigned channels, unsigned sampleRate, StreamingCallback callback)
{
	context.Prepare();
	...
	result->sampleCount = unsigned(sampleCount);
	result->sampleRate = sampleRate;
	result->channels = channels;
	result->streamingCallback = callback;
	size_t size = result->SizeInBytes();
	result->temp_buffer = GlobalHeap.Allocate(size, INTRA_SOURCE_INFO);
	return result;
}
...
void StreamedBufferDelete(StreamedBufferHandle snd)
{
	...
	GlobalHeap.Free(snd->temp_buffer, snd->SizeInBytes());
}
...
static void load_buffer(StreamedBufferHandle snd, size_t index)
{
	...
	if(samplesProcessed < snd->sampleCount)
	{
		...
		FillZeros(Take(endOfData, totalSamplesInBuffer));
		snd->stop_soon = true;
	}
...
void StreamedSoundUpdate(StreamedBufferHandle snd)
{
	...
	while(countProcessed--!=0)
	{
		...
		Swap(snd->buffers[0], snd->buffers[1]);
	}
}
