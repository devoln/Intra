...
{
	enum: unsigned {c1 = 0xcc9e2d51U, c2 = 0x1b873593U, r1 = 15, r2 = 13, m = 5, n = 0xe6546b64U};
...
{
	enum: unsigned {m = 0x5bd1e995};
	enum: unsigned {r = 24};

	unsigned h1 = seed ^ unsigned(key.Length());
	unsigned h2 = 0;
	...
	{
	case 3: h2 ^= unsigned(bytes[2] << 16);
	case 2: h2 ^= unsigned(bytes[1] << 8);
	case 1: h2 ^= unsigned(bytes[0]);
		h2 *= m;
...
inline unsigned getblock(const unsigned* p, index_t i)
{
...
inline unsigned fmix(unsigned h)
{
	h ^= h >> 16;
	h *= 0x85ebca6b;
	h ^= h >> 13;
...
hash128 Murmur3_128_x32(StringView key, unsigned seed)
{
	...
	enum: unsigned {c1 = 0x239b961bU, c2 = 0xab0e9789U, c3 = 0x38b34ae5U, c4 = 0xa1e38b93U};

	const unsigned* blocks = reinterpret_cast<const unsigned*>(data + nblocks*16);
	...
	{
	case 15: k4 ^= unsigned(tail[14]) << 16u;
	case 14: k4 ^= unsigned(tail[13]) << 8u;
	case 13: k4 ^= tail[12];
		k4 *= c4;
		k4  = ROTL32(k4, 18);
		k4 *= c1;
	...
	case 12: k3 ^= unsigned(tail[11]) << 24u;
	case 11: k3 ^= unsigned(tail[10]) << 16u;
	case 10: k3 ^= unsigned(tail[9]) << 8u;
	case 9: k3 ^= tail[8];
		k3 *= c3;
		k3  = ROTL32(k3, 17);
		k3 *= c4;
	...
	case 8: k2 ^= unsigned(tail[7]) << 24u;
	case 7: k2 ^= unsigned(tail[6]) << 16u;
	case 6: k2 ^= unsigned(tail[5]) << 8u;
	case 5: k2 ^= tail[4];
		k2 *= c2;
		k2  = ROTL32(k2, 16);
		k2 *= c3;
	...
	case 4: k1 ^= unsigned(tail[3]) << 24u;
	case 3: k1 ^= unsigned(tail[2]) << 16u;
	case 2: k1 ^= unsigned(tail[1]) << 8u;
	case 1: k1 ^= tail[0];
		k1 *= c1;
		k1  = ROTL32(k1, 15);
		k1 *= c2;
	...
	h1 ^= unsigned(key.Length());
	h2 ^= unsigned(key.Length());
	h3 ^= unsigned(key.Length());
	h4 ^= unsigned(key.Length());
