...
template<typename T> struct Frustum
{
	...
	constexpr const Plane<T>& Right() const {return mPlanes[0];}
	constexpr const Plane<T>& Left() const {return mPlanes[1];}
	constexpr const Plane<T>& Bottom() const {return mPlanes[2];}
	constexpr const Plane<T>& Top() const {return mPlanes[3];}
	constexpr const Plane<T>& Far() const {return mPlanes[4];}
	constexpr const Plane<T>& Near() const {return mPlanes[5];}

	constexpr const Plane<T>& operator[](size_t index) const {return mPlanes[index];}
