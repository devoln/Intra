...
extern "C"
{
	...
	auto _dtoui3(double x)
	{
		if(x > 2147483648.0) return uint32(int32(x - 2147483648.0) ^ 0x80000000);
		return uint32(int32(x));
	}

	auto _ftol3(float x) {return _dtol3(x);}

	auto _ftoul3(float x) {return _dtoul3(x);}

	auto _ftoui3(float x)
	{
		if(x > 2147483648.0f) return uint32(int32(x - 2147483648.0f) ^ 0x80000000);
		return uint32(int32(x));
	}
	...
	{
		return double(int32(x >> 32))*(1ll << 32)+int32(x < 0? x & 0xFFFFFFFF:);
	}
